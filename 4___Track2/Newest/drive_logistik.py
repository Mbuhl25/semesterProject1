...
class Drive:
    def __init__(self, left, right, actoator, steps_per_revolution = 50, wheel_circumference = 26.7, wheelbase = 23.5):
        """
    ...
    def stop(self):
        ...
        self.actuator.stop_step(self.actuator_pins)
    ...
    def move_actuator(self, delay, step, step_sequence, direction):
        for i in range(step):
            seq = step_sequence[self.actuator_seq_index]
            self.actuator.set_duty(self.actuator_pins, seq)
            sleep(delay)
            self.actuator_seq_index = (self.actuator_seq_index + direction) % len(step_sequence)
